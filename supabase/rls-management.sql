...
-- 用户数据访问策略
CREATE POLICY IF NOT EXISTS "Users can only access their own data" ON yt_users
FOR ALL USING (auth.uid()::text = id::text);

-- 频道数据访问策略
CREATE POLICY IF NOT EXISTS "Users can only access their own channels" ON yt_channels
FOR ALL USING (auth.uid()::text = user_id::text);

-- 视频数据访问策略
CREATE POLICY IF NOT EXISTS "Users can only access their own videos" ON yt_videos
FOR ALL USING (auth.uid()::text = user_id::text);

-- 评论数据访问策略（允许查看所有评论，但只能修改自己的）
CREATE POLICY IF NOT EXISTS "Users can view all comments" ON yt_comments
FOR SELECT USING (true);

-- 采集任务访问策略
CREATE POLICY IF NOT EXISTS "Users can only access their own tasks" ON yt_scraping_tasks
FOR ALL USING (auth.uid()::text = user_id::text);

-- AI分析结果访问策略
CREATE POLICY IF NOT EXISTS "Users can only access their own analysis" ON yt_ai_analysis
FOR ALL USING (auth.uid()::text = user_id::text);

-- 分析数据访问策略
CREATE POLICY IF NOT EXISTS "Users can only access their own analytics" ON yt_analytics
FOR ALL USING (auth.uid()::text = user_id::text);

-- 洞察数据访问策略
CREATE POLICY IF NOT EXISTS "Users can only access their own insights" ON yt_insights
FOR ALL USING (auth.uid()::text = user_id::text);

-- 报告数据访问策略
CREATE POLICY IF NOT EXISTS "Users can only access their own reports" ON yt_reports
FOR ALL USING (auth.uid()::text = user_id::text);

-- 仪表板数据访问策略
CREATE POLICY IF NOT EXISTS "Users can only access their own dashboards" ON yt_dashboards
FOR ALL USING (auth.uid()::text = user_id::text);

-- AI洞察访问策略
CREATE POLICY IF NOT EXISTS "Users can only access their own ai insights" ON yt_ai_insights
FOR ALL USING (auth.uid()::text = user_id::text);

-- 竞争对手分析访问策略
CREATE POLICY IF NOT EXISTS "Users can only access their own competitor analysis" ON yt_competitor_analysis
FOR ALL USING (auth.uid()::text = user_id::text);

-- 团队数据访问策略
CREATE POLICY IF NOT EXISTS "Team owners can manage their teams" ON yt_teams
FOR ALL USING (auth.uid()::text = owner_id::text);

CREATE POLICY IF NOT EXISTS "Team members can view their teams" ON yt_teams
FOR SELECT USING (
    EXISTS (
...
-- 团队成员数据访问策略
CREATE POLICY IF NOT EXISTS "Team members can view team membership" ON yt_team_members
FOR SELECT USING (
...
-- 团队邀请访问策略
CREATE POLICY IF NOT EXISTS "Team owners can manage invitations" ON yt_team_invitations
FOR ALL USING (
    EXISTS (
        SELECT 1 FROM yt_teams 
...
-- 协作评论访问策略
CREATE POLICY IF NOT EXISTS "Team members can manage comments" ON yt_collaboration_comments
FOR ALL USING (
    EXISTS (
        SELECT 1 FROM yt_team_members 
...
-- 匿名试用访问策略（基于设备指纹）
CREATE POLICY IF NOT EXISTS "Anonymous users can access their own trials" ON yt_anonymous_trials
FOR ALL USING (
...
CREATE POLICY IF NOT EXISTS "Users can view their own login analytics" ON yt_login_analytics
FOR SELECT USING (
